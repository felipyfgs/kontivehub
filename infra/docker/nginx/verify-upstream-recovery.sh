...
validate_configuration infra/docker/nginx/conf/dev.conf
validate_configuration infra/docker/nginx/conf/prod.conf
...
if [ -z "$new_php_id" ] || [ -z "$new_php_ip" ] || [ "$new_php_ip" = "$old_php_ip" ]; then

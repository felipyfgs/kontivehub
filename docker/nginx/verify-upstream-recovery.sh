...
validate_configuration docker/nginx/conf/development.conf

nginx_id=$(container_id nginx)
php_id=$(container_id api)

if [ -z "$nginx_id" ] || [ -z "$php_id" ]; then
    echo "Os serviços nginx e api precisam estar em execução." >&2
    exit 1
fi
...
compose stop -t 10 api >/dev/null
...
compose up -d --no-deps api >/dev/null
new_php_id=$(container_id api)

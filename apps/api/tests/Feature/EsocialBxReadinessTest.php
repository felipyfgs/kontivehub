...
class EsocialBxReadinessTest extends TestCase
{
    ...
    public function test_readiness_is_tenant_scoped_and_ready_with_active_certificate(): void
    {
        $tenant = Tenant::factory()->create();
        $client = Client::factory()->forTenant($tenant)->create();
        $credential = $this->credential($client);

        $readiness = app(EsocialBxReadinessService::class)->check($tenant, $client);
    ...
    public function test_window_and_conservative_quota_block_before_egress(): void
    {
        ...
        $quota = app(EsocialBxReadinessService::class)->check($tenant, $client);
        ...
        $window = app(EsocialBxReadinessService::class)->check($tenant, $client);
        $this->assertContains('ESOCIAL_BX_BLOCKED_WINDOW', array_column($window->blockers, 'code'));
    }

    public function test_readiness_endpoint_is_sanitized_and_cannot_cross_tenant(): void
    {
        $tenant = Tenant::factory()->create();
        $other = Tenant::factory()->create();
        $client = Client::factory()->forTenant($tenant)->create();
        ...
        $this->credential($client);
        Sanctum::actingAs(User::factory()->forTenant($tenant, TenantRole::TenantUser)->create());
    ...
    public function test_missing_expired_future_and_mismatched_credentials_are_reported_without_materialization(): void
    {
        ...
        $missing = $service->check($tenant, $client);
        ...
        $expiredResult = $service->check($tenant, $client);
        ...
        $future = $service->check($tenant, $client);
        ...
        $mismatch = $service->check($tenant, $client);
    ...
    public function test_config_feature_and_kill_switch_block_before_credential_query(): void
    {
        ...
        $this->assertNoCredentialQuery(
            fn () => $service->check($tenant, $client),
        ...
        FiscalModuleControl::query()->create([
            ...
            'updated_by_user_id' => User::factory()->forTenant($tenant)->create()->id,
        ]);
        $this->assertNoCredentialQuery(
            fn () => $service->check($tenant, $client),

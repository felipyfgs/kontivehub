...
class SitfisPortfolioProjectionTest extends TestCase
{
    ...
    public function test_counts_only_current_sitfis_snapshot_and_sitfis_pending_items(): void
    {
        ...
        $sitfisRun = $this->makeRun($tenant, $client, 'INTEGRA_SITFIS', 'SITFIS');
        $otherRun = $this->makeRun($tenant, $client, 'INTEGRA_DCTFWEB', 'DCTFWEB');
    ...
    public function test_local_reprocess_creates_idempotent_successor_without_remote_call(): void
    {
        ...
        $tenant = Tenant::factory()->create();
        $client = Client::factory()->for($tenant)->create();
        $run = $this->makeRun($tenant, $client, 'INTEGRA_SITFIS', 'SITFIS');
        ...
        $this->finding($source, 'SITFIS_STALE_FINDING');
        $this->pending($run, $source, 'SITFIS_STALE_FINDING');
        $otherRun = $this->makeRun($tenant, $client, 'INTEGRA_DCTFWEB', 'DCTFWEB');
        $this->pending($otherRun, $source, 'OTHER_MODULE_PENDING');

        $service = app(SitfisSnapshotReprocessService::class);
        $first = $service->reprocess((int) $tenant->id, (int) $client->id, true);
        $second = $service->reprocess((int) $tenant->id, (int) $client->id, true);
        ...
        $this->assertFalse(FiscalFinding::query()->withoutGlobalScopes()->where('code', 'SITFIS_STALE_FINDING')->firstOrFail()->is_active);
        $this->assertSame(FiscalPendingStatus::Resolved, FiscalPendingItem::query()->withoutGlobalScopes()->where('code', 'SITFIS_STALE_FINDING')->firstOrFail()->status);
    ...
    private function makeRun(Tenant $tenant, Client $client, string $system, string $service): FiscalMonitoringRun
    {
    ...
    private function snapshot(FiscalMonitoringRun $run, bool $current, int $version): FiscalSnapshot
    {
        return FiscalSnapshot::query()->withoutGlobalScopes()->create([
            'tenant_id' => $run->tenant_id, 'run_id' => $run->id, 'client_id' => $run->client_id,
    ...
    private function finding(FiscalSnapshot $snapshot, string $code): void
    {
        FiscalFinding::query()->withoutGlobalScopes()->create([
            'tenant_id' => $snapshot->tenant_id, 'snapshot_id' => $snapshot->id,
    ...
    private function pending(FiscalMonitoringRun $run, FiscalSnapshot $snapshot, string $code): void
    {
        FiscalPendingItem::query()->withoutGlobalScopes()->create([
            'tenant_id' => $run->tenant_id, 'client_id' => $run->client_id,

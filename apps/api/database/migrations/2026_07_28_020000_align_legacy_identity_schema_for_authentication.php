...
{
    private const ADDED_COLUMNS_TABLE = 'legacy_identity_compatibility_columns';

    private const PERMISSION_PROFILE_COMPOSITE_UNIQUE = 'legacy_identity_permission_profiles_id_tenant_unique';

    private const PERMISSION_PROFILE_COMPOSITE_UNIQUE_MARKER = '@index:legacy_identity_permission_profiles_id_tenant_unique';

    private const TENANT_MEMBERSHIP_PERMISSION_PROFILE_FOREIGN = 'legacy_identity_membership_permission_profile_fk';
    ...
    {
        if (! $this->usesLegacyStatus('users')) {
            return;
        }
    ...
    {
        if (! $this->usesLegacyStatus('tenants')) {
            return;
        }
    ...
    {
        if (! $this->usesLegacyStatus('tenant_memberships')) {
            return;
        }
    ...
    {
        if (! $this->usesLegacyStatus('platform_memberships')) {
            return;
        }
    ...
    {
        if (! $this->usesLegacyStatus('platform_settings')) {
            return;
        }
    ...
    private function usesLegacyStatus(string $table): bool
    {
    ...
    private function assertNoPermissiveHybridConflicts(): void
    {
        foreach (['users', 'tenants'] as $table) {
            if (
                ...
                throw new RuntimeException(
                    "Conflito permissivo entre status legado e is_active em {$table}.",
                );
            }
        }
        ...
        if (
            ...
            throw new RuntimeException(
                'Conflito permissivo entre status legado e is_active em tenant_memberships.',
            );
        }
        ...
        if (
            ...
            throw new RuntimeException(
                'Conflito permissivo entre status legado e is_active em platform_memberships.',
            );
        }
    }

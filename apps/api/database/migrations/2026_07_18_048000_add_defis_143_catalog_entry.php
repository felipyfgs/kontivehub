...
/** Coordenada oficial DEFIS / CONSULTIMADECREC143. */
return new class extends Migration
{
    public function up(): void
    {
        ...
        foreach (['TRIAL', 'HOMOLOGATION', 'PRODUCTION'] as $environment) {
            DB::table('serpro_service_catalog_entries')->updateOrInsert([
                ...
                'label' => 'Consultar última declaração DEFIS e recibo',
                'is_mutating' => false,
                'is_enabled' => true,
        ...
        if (Schema::hasTable('serpro_operation_catalog')) {
            DB::table('serpro_operation_catalog')->updateOrInsert([
                'operation_key' => 'defis.consultimadecrec',
                'system_code' => 'DEFIS',
                'service_code' => 'CONSULTIMADECREC143',
            ], [
                'operation_code' => 'defis.consultimadecrec',
                'consumption_class' => 'CONSULTA',
                'is_essential' => true,
                'effective_from' => $now,
                'effective_to' => null,
                'label' => 'Consultar última DEFIS e recibo',
                'notes' => 'Coordenada oficial DEFIS/CONSULTIMADECREC143.',
                'updated_at' => $now,
                'created_at' => $now,
            ]);
        }
    ...
    {
        if (Schema::hasTable('serpro_service_catalog_entries')) {
            DB::table('serpro_service_catalog_entries')->where('operation_key', 'defis.consultimadecrec')->delete();
        }
        if (Schema::hasTable('serpro_operation_catalog')) {
            DB::table('serpro_operation_catalog')->where('operation_key', 'defis.consultimadecrec')->delete();
        }
    }
};

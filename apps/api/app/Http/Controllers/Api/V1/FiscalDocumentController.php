...
class FiscalDocumentController extends Controller
{
    public function index(
        ...
        return response()->json($catalog->index($request, $currentTenant));
    }
    ...
    public function insights(
        ...
        return response()->json($catalog->insights($request, $currentTenant));
    }
    ...
    public function byClient(
        ...
        return response()->json($catalog->byClient($request, $currentTenant));
    }

    public function show(string $accessKey, DocumentCatalogService $catalog): JsonResponse
    {
        $nfseDocument = NfseDocument::query()->where('access_key', $accessKey)->with('document')->first();
        if ($nfseDocument !== null) {
            ...
            $payload = $catalog->serializeNfseListItem($nfseDocument);
            $payload['dfe_document_id'] = $nfseDocument->dfe_document_id;
            $payload['has_full_xml'] = true;
            $payload['xml_completeness'] = 'FULL';

            return response()->json([
                'data' => [
                    'fiscal_document' => $payload,
                    'events' => $events->map(static fn (NfseEvent $event): array => [
                        'id' => $event->id,
                    ...
                    'metadata' => [
                        'id' => $nfseDocument->document?->id,
                        'sha256' => $nfseDocument->document?->sha256,
                        'schema_version' => $nfseDocument->document?->schema_version,
                        'parse_status' => $nfseDocument->document?->parse_status,
                        'parse_alert' => $nfseDocument->document?->parse_alert,
                        'byte_size' => $nfseDocument->document?->byte_size,
                        'document_type' => $nfseDocument->document?->document_type,
                    ],
                ],
            ]);
        }
     ...
     * Sem permissão: 403. Full já presente: no-op 200.
     */
    public function unlockXml(
        ...
        $result = $unlock->unlock($accessKey, $currentTenant->tenant()->id);
    ...
    /**
     * Manifestação do destinatário (ciência / conclusivas). Sem permissão: 403.
     */
    ...
    private function streamCanonicalXml(
        ...
        $note = NfseDocument::query()->where('access_key', $accessKey)->with('document')->first();
        if ($note !== null) {
            $doc = $note->document;
            if ($doc === null) {

...
class OperationalProcessController extends Controller
{
    ...
    public function index(Request $request, CurrentOffice $currentOffice): JsonResponse
    {
        ...
        if ($request->boolean('without_template') && $request->filled('process_template_id')) {
            throw ValidationException::withMessages([
                'without_template' => ['Não combine without_template com process_template_id.'],
            ]);
        }
        ...
        $q = OperationalProcess::query()
            ->with(array_values(array_filter([
                ...
                $includeTasks ? 'tasks.assigneeMembership:id,user_id,office_id' : null,
        ...
        if ($request->boolean('without_template')) {
            $q->whereNull('process_template_id');
        } elseif ($request->filled('process_template_id')) {
            $q->where('process_template_id', (int) $request->input('process_template_id'));
        }
        ...
        return response()->json([
            'data' => collect($paginator->items())->map(
                fn (OperationalProcess $p) => $this->public($p, false, $today, $includeTasks)
            ),
            'meta' => [
     ...
     */
    private function referencePeriodPayload(OperationalProcess $p): ?array
    {

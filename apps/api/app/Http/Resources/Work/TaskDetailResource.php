...
final class TaskDetailResource extends JsonResource
{
    ...
    public function toArray(Request $request): array
    {
        ...
        $data = TaskResource::make($task)->resolve($request);

        $data['risks'] = $detail->risks;
        $data['effective_due_date'] = $detail->effectiveDueDate;
        $data['bucket'] = $detail->bucket;
        $data['evidences'] = TaskEvidenceResource::collection(
            $task->evidences,
        )->resolve($request);
        $data['comments'] = TaskCommentResource::collection(
            $task->comments,
        )->resolve($request);
        if ($process !== null) {
            $data['process'] = [

...
final class FlowDryRunService
{
    ...
    public function __construct(
        ...
        private readonly FlowTextMasker $masker,
    ) {}
     ...
     * }  $context
     */
    public function simulate(array $graph, int $tenantId, array $context = []): FlowDryRunResult
    {
        $validation = $this->validator->validate($graph, $tenantId);
        if (! $validation->valid) {
            return FlowDryRunResult::invalid($validation->digest, $validation->errors);
        }
        ...
        if ($startId === null) {
            return FlowDryRunResult::invalid($digest, [[
                'path' => 'graph.nodes',
                'code' => 'start_required',
        ...
        return FlowDryRunResult::ok($digest, $outcome, $steps);
    }

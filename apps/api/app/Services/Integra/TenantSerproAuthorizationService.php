...
use App\Models\TenantSerproAuthorizationEvent;
 ...
 */
final class TenantSerproAuthorizationService
{
    ...
    public function getOrCreate(Tenant $tenant, SerproEnvironment $environment): TenantSerproAuthorization
    {
    ...
    public function configureAuthor(
        ...
        $this->assertTenantEligibleForEnvironment($tenant, $environment);
        $identity = $this->normalizeIdentity($identity);
        $this->assertIdentity($identityType, $identity);

        $auth = $this->getOrCreate($tenant, $environment);
        $from = $auth->status;
    ...
    /**
     * Gera draft canônico não assinado e armazena no vault (fluxo externo certificado/A3).
     *
     * @return array{auth: TenantSerproAuthorization, draft_sha256: string}
     */
     ...
     */
    public function getTermoDraftXml(Tenant $tenant, SerproEnvironment $environment): string
    {
        $auth = $this->getOrCreate($tenant, $environment);
        $meta = is_array($auth->metadata) ? $auth->metadata : [];
    ...
    public function uploadTermo(
        ...
        return DB::transaction(function () use ($tenant, $environment, $termoXml, $actorUserId, $auth, $validation) {
            ...
            $versionNumber = (int) SerproTermVersion::query()
                ->where('tenant_serpro_authorization_id', $auth->id)
    ...
    /**
     * Dispara assinatura com certificado gerenciado (consentimento versionado + job dedicado).
     */
    public function dispatchManagedCertificateSign(
        Tenant $tenant,
        SerproEnvironment $environment,
        bool $consent,
        ?int $actorUserId = null,
        ...
        if (! $consent) {
            throw new RuntimeException('Consentimento versionado é obrigatório para assinar com certificado gerenciado.');
        }
        if ($actorUserId === null) {
            throw new RuntimeException('Ator administrador com senha recente é obrigatório.');
        }

        $auth = $this->getOrCreate($tenant, $environment);
        if ($auth->certificate_mode !== AuthorCertificateMode::ManagedCertificate) {
            throw new RuntimeException('Configure o certificado gerenciado antes de assinar.');
        }

        $credential = TenantCredentialPurposeLink::query()
            ->where('tenant_id', $tenant->id)
        ...
        if ($credential === null || ! $credential->status->isUsable()) {
            throw new RuntimeException('certificado ativo e vinculado à assinatura do Termo é obrigatório.');
        }

        $meta = is_array($auth->metadata) ? $auth->metadata : [];
        if (empty($meta['termo_draft_vault_object_id'])) {
            // Auto-gerar draft se ausente.
            $this->generateTermoDraft($tenant, $environment, null, $actorUserId);
            $auth = $auth->refresh();
        }
        ...
        SerproAuthorizationConsent::query()->create([
            'tenant_id' => $tenant->id,
            'tenant_serpro_authorization_id' => $auth->id,
            'consent_type' => SerproAuthorizationConsent::TYPE_CERTIFICATE_SIGN,
            'version_code' => SerproAuthorizationConsent::VERSION_CERTIFICATE_SIGN_V1,
            'actor_user_id' => $actorUserId,
            'consented_at' => now(),
            'payload_sha256' => hash(
                'sha256',
                $credential->fingerprint_sha256.'|sign|'.SerproAuthorizationConsent::VERSION_CERTIFICATE_SIGN_V1,
            ),
            'metadata' => ['action' => 'sign_termo'],
        ]);

        SignTermoWithManagedCertificateJob::dispatch(
            $tenant->id,
            $environment->value,
            $auth->id,
            $actorUserId,
        ...
        $this->audit->record('serpro.authorization.termo_managed_certificate_dispatch', 'SUCCESS', $auth, [
            'environment' => $environment->value,
        ], $actorUserId, $tenant->id);
    ...
    public function refreshProcuradorToken(
        ...
        if (FiscalProfile::configured() === FiscalProfile::Dev) {
            return $this->activateDevFixtureAuthorization($tenant, $environment, $actorUserId);
        }
        ...
        try {
            $purged = app(SerproOperationAttemptStore::class)
                ->purgeNonStickyTokenFailures((int) $tenant->id, $environment->value);
            if ($purged > 0) {
    ...
    public function invalidateDerivedAuthorization(
        ...
        // Snapshots não podem mascarar o ensure pré-consulta após troca/remoção de certificado.
    ...
    private function recordEvent(
        ...
        TenantSerproAuthorizationEvent::query()->create([
    ...
    /**
     * Bloqueia Tenant demo no ambiente produtivo.
     */
    public function assertTenantEligibleForEnvironment(Tenant $tenant, SerproEnvironment $environment): void
    {
        app(SerproProductionOnboardingGuard::class)
            ->assertMayUseRealEndpoint($tenant, $environment);
    }
}

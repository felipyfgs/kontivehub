...
/**
 * Configuração office-scoped da agenda de recorrência da Rotina.
 */
final class ProcessTemplateRecurrenceService
{
     ...
     */
    public function toPublic(ProcessTemplate $template): array
    {
        return [
     ...
     * @return array<string, mixed>
     */
    public function attributesFromInput(array $input, ?ProcessTemplate $existing = null): array
    {
        ...
        if ($schedule->enabled) {
            $attrs['next_run_at'] = $schedule->upcomingRunAtUtc($this->currentOffice->office());
        } else {
    ...
    public function update(ProcessTemplate $template, array $input): ProcessTemplate
    {
        ...
        if ($officeId === null || (int) $template->office_id !== (int) $officeId) {
            abort(404);
        }

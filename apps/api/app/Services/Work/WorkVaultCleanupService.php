 ...
 */
final class WorkVaultCleanupService
{
    ...
    {
        $count = 0;
        WorkProcessGenerationBatch::query()
            ->where('status', GenerationBatchStatus::Previewed->value)
            ->whereNotNull('expires_at')
            ->where('expires_at', '<', now())
            ->orderBy('id')
            ->each(function (WorkProcessGenerationBatch $batch) use (&$count): void {
    ...
    public function expireExports(): int
    {
        $count = 0;
        WorkExport::query()
            ->whereIn('status', [WorkExportStatus::Ready->value, WorkExportStatus::Pending->value])
            ->whereNotNull('expires_at')
            ->where('expires_at', '<', now())
            ->orderBy('id')
            ->each(function (WorkExport $export) use (&$count): void {
                ...
                $export->forceFill([
                    'status' => WorkExportStatus::Expired,
                    'storage_path' => null,
                ])->save();
                $count++;
            });
    ...
    {
        $count = 0;
        WorkTaskEvidence::query()
            ->whereNotNull('removed_at')
            ->whereNotNull('vault_object_id')
            ->where('removed_at', '<', now()->subDays(7))
            ->orderBy('id')
            ->each(function (WorkTaskEvidence $evidence) use (&$count): void {
                try {
                    ...
                    Log::info('work.evidence.vault_purged', [
                        'purpose' => SecureObjectPurpose::WorkTaskEvidence->value,

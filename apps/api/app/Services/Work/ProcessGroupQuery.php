...
/**
 * Agregação tenant-scoped de processos por cliente ou rotina (SQL).
 * Não materializa a lista de processos na resposta de grupos.
 */
final class ProcessGroupQuery
{
    ...
    public function paginate(
        ...
        $query = $this->buildGroupedQuery($tenantId, $groupBy, $filters);
        ...
        $items = $this->hydrateRows($rows, $groupBy, $tenantId);
     ...
     */
    private function buildGroupedQuery(int $tenantId, string $groupBy, array $filters): Builder
    {
        $terminal = [
        ...
        foreach (ProcessStatus::cases() as $status) {
            ...
            $statusSelects[] = DB::raw(
                "SUM(CASE WHEN p.status = '{$value}' THEN 1 ELSE 0 END) as status_{$alias}"
            );
        }

        $query = DB::table('work_processes as p')
            ->leftJoinSub($taskAgg, 't', 't.work_process_id', '=', 'p.id')
        ...
        $aggregates = array_merge([
            ...
            // Mesma regra do hydrate: zero quando não há tarefas.
            DB::raw(
        ...
        if ($groupBy === 'client') {
            ...
            $query->leftJoin('work_process_templates as pt', 'pt.id', '=', 'p.work_process_template_id')
                ->select(array_merge([
                    DB::raw(
                        "CASE WHEN p.work_process_template_id IS NULL THEN '".self::MANUAL_KEY."' "
                        .'ELSE CAST(p.work_process_template_id AS TEXT) END as group_key'
                    ),
                    DB::raw(
                        "CASE WHEN p.work_process_template_id IS NULL THEN '".self::MANUAL_LABEL."' "
                        .'ELSE pt.name END as label'
                    ),
                    'p.work_process_template_id as entity_id',
                ], $aggregates))
                ->groupBy('p.work_process_template_id', 'pt.name');
        }
     ...
     */
    private function hydrateRows(Collection $rows, string $groupBy, int $tenantId): array
    {
        if ($rows->isEmpty()) {
            return [];
        }
        ...
        if ($groupBy === 'client') {
            ...
            $clients = Client::query()
                ->where('tenant_id', $tenantId)
                ->whereIn('id', $ids)
                ->with(['establishments:id,client_id,cnpj,is_headquarters'])
                ->get()
                ->keyBy('id');
    ...
    private function statusCountsFromRow(object $row): array
    {
        $counts = [];
        foreach (ProcessStatus::cases() as $status) {
            $prop = 'status_'.strtolower($status->value);

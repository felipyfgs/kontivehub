 ...
 */
final class ProcessGenerationService
{
    ...
    public function preview(
        ...
        ProcessGenerationPreviewData $data,
        ...
        if (! $template->is_active || (int) $template->tenant_id !== (int) $tenant->id) {
            throw ValidationException::withMessages([
                'work_process_template_id' => ['Modelo inativo ou inexistente neste escritório.'],
            ]);
        }
        ...
        if ($template->tasks->isEmpty()) {
            throw ValidationException::withMessages([
                'work_process_template_id' => ['Modelo sem tarefas padrão.'],
            ]);
        }

        $audience = $this->audiences->resolve($template, $period->value(), $selection);
        ...
        foreach ($audience['items'] as $selectedClient) {
            ...
            try {
                foreach ($template->tasks as $tt) {
                    /** @var WorkProcessTemplateTask $tt */
                    $taskDue = null;
        ...
        return DB::transaction(function () use (
            ...
            $batch = WorkProcessGenerationBatch::query()->create([
                'tenant_id' => $tenant->id,
                'work_process_template_id' => $template->id,
                'template_lock_version' => $template->lock_version,
                'competence' => $period->value(),
            ...
            foreach ($itemsPayload as $item) {
                WorkProcessGenerationItem::query()->create([
                    'tenant_id' => $tenant->id,
                    'batch_id' => $batch->id,
                    'client_id' => $item['client_id'],
    ...
    public function confirm(WorkProcessGenerationBatch $batch, ?string $idempotencyKey = null): WorkProcessGenerationBatch
    {
        $tenantId = $this->currentTenant->id();
        if ((int) $batch->tenant_id !== (int) $tenantId) {
            abort(404);
        }
     ...
     */
    public function retryFailedItems(WorkProcessGenerationBatch $batch): WorkProcessGenerationBatch
    {
        $tenantId = $this->currentTenant->id();
        if ((int) $batch->tenant_id !== (int) $tenantId) {
            abort(404);
        ...
        $template = WorkProcessTemplate::query()->findOrFail($batch->work_process_template_id);
        if (! $template->is_active || (int) $template->tenant_id !== (int) $tenantId) {
            throw ValidationException::withMessages([
    ...
    public function processBatch(WorkProcessGenerationBatch $batch): void
    {
    ...
    private function materializeItem(WorkProcessGenerationBatch $batch, WorkProcessGenerationItem $item): void
    {
        DB::transaction(function () use ($batch, $item): void {
            ...
            $process = WorkProcess::query()->create([
                ...
                'template_snapshot' => [
                    'template_id' => $batch->work_process_template_id,
    ...
    private function periodFromBatch(WorkProcessGenerationBatch $batch): ReferencePeriod
    {
        try {

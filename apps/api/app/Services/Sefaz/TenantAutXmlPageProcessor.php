 ...
 */
final class TenantAutXmlPageProcessor
{
    ...
    public function process(
        ...
        DistDfePageDto $page,
        ?TenantDistributionRun $run = null,
    ): array {
        if ($page->isAbuse()) {
        ...
        try {
            return DB::transaction(function () use ($cursor, $page, $run, &$storedObjectIds) {
                $cursor = TenantDistributionCursor::query()->whereKey($cursor->id)->lockForUpdate()->firstOrFail();
            ...
            if ($e instanceof DocumentDecodeException) {
                $fresh = TenantDistributionCursor::query()->whereKey($cursor->id)->first();
                if ($fresh) {
    ...
    private function persistNote(
        ...
        if ($recipient !== null && $recipient === $queryCnpj) {
            $this->quarantine($cursor, $doc, $decoded, $sha, $accessKey, $issuer, $recipient, $family, QuarantineReason::TenantAlsoRecipient, $storedObjectIds);
        ...
        $enrollment = TenantAutXmlEnrollment::query()
            ->where('tenant_id', $cursor->tenant_id)
            ->where('tenant_fiscal_identity_id', $cursor->tenant_fiscal_identity_id)
            ->where('establishment_id', $issuerEstab->id)
            ->whereIn('status', [
                TenantAutXmlEnrollmentStatus::Pending->value,
                TenantAutXmlEnrollmentStatus::Confirmed->value,
            ])
            ->first();

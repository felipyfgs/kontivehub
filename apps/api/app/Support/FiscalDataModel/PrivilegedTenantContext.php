 ...
 * Permite consultas de tenant sem CurrentTenant, com motivo auditável.
 ...
 */
final class PrivilegedTenantContext
{
    ...
    public static function run(string $reason, callable $callback): mixed
    {
        $reason = trim($reason);
        if ($reason === '') {
            throw new RuntimeException('PrivilegedTenantContext exige motivo não vazio.');
        }
    ...
    public static function enter(string $reason): void
    {
        ...
        if (self::$depth === 1) {
            Log::debug('privileged_tenant_context.enter', [
                'reason' => $reason,
            ]);
        }
    }
    ...
    public static function leave(): void
    {
        ...
        self::$depth--;
        if (self::$depth === 0) {
            Log::debug('privileged_tenant_context.leave', [
                'reason' => self::$reason,
            ]);
            self::$reason = null;
        }

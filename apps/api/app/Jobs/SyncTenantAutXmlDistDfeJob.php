...
use App\Services\Sefaz\TenantAutXmlPageProcessor;
 ...
 */
class SyncTenantAutXmlDistDfeJob implements ShouldQueue
{
    use Queueable;
    ...
    public function __construct(
        public int $tenantDistributionCursorId,
    ...
    public function handle(
        ...
        TenantAutXmlPageProcessor $processor,
        ...
        $cursor = TenantDistributionCursor::query()->find($this->tenantDistributionCursorId);
        if ($cursor === null || $cursor->channel !== CaptureChannel::NfeAutXmlDistDfe) {
            return;
        }

        if (! AutXmlFeature::isTenantAllowed((int) $cursor->tenant_id)) {
            return;
        }
        ...
        $lockKey = 'sefaz:autxml:root:'.$cursor->tenant_id.':'.$cursor->interested_root_cnpj.':'.$cursor->environment;
        ...
        try {
            ...
            $run = TenantDistributionRun::query()->create([
                'tenant_id' => $cursor->tenant_id,
            ...
            $resolved = $resolver->resolveForAutXml((int) $cursor->tenant_id);
            $material = $resolved['material'];
            // Nunca usar ClientCredential — resolver já garante TenantCredential.
            $identityCnpj = strtoupper($resolved['identity']->cnpj);
            $queryCnpj = strtoupper($cursor->query_cnpj);

            // Garante que o pedido DistDFe usa a mesma raiz do certificado do escritório (evita cStat 656).
            ...
            Log::error('sefaz.autxml.job.error', [
                'cursor_id' => $this->tenantDistributionCursorId,

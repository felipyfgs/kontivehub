...
use App\Services\Sefaz\TenantCteAutXmlPageProcessor;
 ...
 * Reutiliza TenantCredential (finalidade NFE_AUTXML_DISTDFE); nunca ClientCredential.
 */
class SyncTenantCteAutXmlDistDfeJob implements ShouldQueue
{
    use Queueable;
    ...
    public function handle(
        ...
        TenantCteAutXmlPageProcessor $processor,
        ...
        if (! CteAutXmlFeature::isTenantAllowed((int) $cursor->tenant_id)) {
            return;
        }
        ...
        $lockKey = 'sefaz:cte_autxml:root:'.$cursor->tenant_id.':'.$cursor->interested_root_cnpj.':'.$cursor->environment;

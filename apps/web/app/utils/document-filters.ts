...
const SELECT_KEYS = new Set<keyof DocumentFilterState>([
  'kind',
  'direction',
  'client_id',
  'establishment_id',
...
export function emptyDocsFilters(): DocumentFilterState {
  return {
    q: '',
    kind: FILTER_ALL,
...
const FILTER_KEYS: (keyof DocumentFilterState)[] = [
  'q',
  'kind',
  'direction',
  'client_id',
...
export function activeTriageQueue(filters: DocumentFilterState): NotesTriageQueue {
...
/** DocumentExport filters espelhando o catálogo (campos que o ZIP job entende). */
export function catalogToExportFilters(filters: DocumentFilterState): {
  access_key?: string
  issuer_cnpj?: string
  taker_cnpj?: string
  competence?: string
...
export function hasExportableCatalogFilters(filters: DocumentFilterState): boolean {
  return Object.keys(catalogToExportFilters(filters)).length > 0
}

export function hasActiveCatalogFilters(filters: DocumentFilterState): boolean {

...
export function onboardingStatusLabel(status?: string | null): string {
  switch (status as TenantOnboardingStatus | string) {
    case 'incomplete':
...
export function onboardingStageIndex(stage?: string | null): number {
  const index = TENANT_ONBOARDING_STAGES.findIndex(item => item.key === stage)
...
export function credentialAlerts(credential: TenantCertificate | null | undefined): string[] {
...
export function actionableTenantError(
  message: string | null | undefined,
  fallback = 'Não foi possível carregar a configuração do escritório.'
): string {
  const raw = (message || '').trim()
  if (!raw) return fallback
  // Sanitiza jargão técnico conhecido se vazar da API.
...
export function emptyOnboarding(): TenantOnboardingActionable {
  return {
    status: 'incomplete',
    actions: [
      { code: 'COMPLETE_PROFILE', label: 'Completar perfil' },
      { code: 'ACCEPT_CONSENT', label: 'Aceitar consentimento' },
      { code: 'UPLOAD_CERTIFICATE', label: 'Enviar certificado' }
    ]
  }
}

...
/**
 * Conteúdo do painel de consumo do tenant.
 * Sem fatura global, custo interno de outros tenants ou orçamento da software house.
 */
import type { TableColumn } from '@nuxt/ui'
import type { TenantUsageEntry, TenantUsageSummary } from '~/types/api'
...
const usage = ref<TenantUsageSummary | null>(null)
const entries = ref<TenantUsageEntry[]>([])
...
const entryColumns: TableColumn<TenantUsageEntry>[] = [
  {
...
async function load() {
  ...
  try {
    const [sumRes, entRes] = await Promise.allSettled([
      api.tenant.serproUsage.summary({ year: year.value, month: month.value }),
      api.tenant.serproUsage.entries({
        year: year.value,
        month: month.value,
        page: page.value,
    ...
    if (entRes.status === 'fulfilled') {
      ...
      entries.value = (body.data as TenantUsageEntry[]) || []

...
import type { TenantCertificate } from '~/types/api'
import { credentialAlerts } from '~/utils/tenant-settings'

const props = withDefaults(defineProps<{
  credential: TenantCertificate | null
  loading?: boolean
  saving?: boolean
  refreshing?: boolean
  readonly?: boolean
...
function submitCredentialForm() {
  const el = document.getElementById('tenant-credential-form') as HTMLFormElement | null
  el?.requestSubmit()
}
...
<template>
  <div data-testid="settings-credential-section">
    ...
    <ShellFormModal
      v-if="!readonly"
      v-model:open="open"
      :title="credential ? 'Substituir certificado' : 'Enviar certificado'"
      ...
      <template #body>
        ...
        >
          ...
          <UCheckbox
            ...
            label="Autorizo o uso deste certificado pelo sistema"

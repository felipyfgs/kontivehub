...
/**
 * Conteúdo da equipe do escritório — Filter Lite (busca ± select de papel; sem chips).
 ...
 */
import type { ActivationMethod, CredentialDeliveryPayload, TenantMember, TenantRole } from '~/types/api'
import { canManageTenantTeam } from '~/utils/permissions'
...
const canMutate = computed(() => canManageTenantTeam(me.value))

const items = ref<TenantMember[]>([])
...
async function load() {
  ...
  try {
    const res = await api.tenant.members.list()
...
async function changeRole(member: TenantMember, role: TenantRole) {
  ...
  try {
    await withPassword(async () => {
      await api.tenant.members.updateRole(member.id, { role })
...
async function deactivate(member: TenantMember) {
  actingId.value = member.id
  try {
    await withPassword(async () => {
      await api.tenant.members.deactivate(member.id)
...
async function reactivate(member: TenantMember) {
  actingId.value = member.id
  try {
    await withPassword(async () => {
      const res = await api.tenant.members.reactivate(member.id, {
...
async function regenerate(member: TenantMember) {
  actingId.value = member.id
  try {
    await withPassword(async () => {
      const res = await api.tenant.members.regenerateActivation(member.id, {
...
<template>
  ...
  >
    ...
    <UEmpty
      ...
      description="Disponível somente a quem pode gerenciar a equipe do escritório."
      class="py-10"

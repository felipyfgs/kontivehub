...
/**
 * Confirmação do administrador do tenant e resultado fiscal DTE no tenant.
...
const isTenantAdmin = computed(() => {
  return me.value?.real_tenant_role === 'tenant_admin'
})
...
async function load() {
  ...
  error.value = null
  try {
    const res = await api.tenant.dteCanary.pending()
    if (seq !== loadSeq || epoch !== sessionEpoch.value) return
    pending.value = res.data
    result.value = null
    if (res.data?.id && res.data.result_status) {
      try {
        const r = await api.tenant.dteCanary.result(Number(res.data.id))
        if (seq !== loadSeq || epoch !== sessionEpoch.value) return
        result.value = r.data
      } catch {
    ...
    if (seq !== loadSeq || epoch !== sessionEpoch.value) return
    error.value = apiErrorMessage(e, 'Falha ao carregar canário DTE do Tenant.')
    pending.value = null
  } finally {
...
async function confirm() {
  ...
  acting.value = true
  try {
    await api.tenant.dteCanary.confirm(id)
    toast.add({ title: 'Participação no canário DTE confirmada', color: 'success' })
    await load()
  } catch (e) {
    toast.add({
      title: 'Confirmação falhou',
      description: apiErrorMessage(e, 'Requer permissão administrativa, senha recente e tenant piloto.'),
      color: 'error'
    })
  } finally {
    acting.value = false
...
<template>
  <UPageCard
    title="Canário DTE (Tenant)"
    description="Confirmação administrativa e resultado fiscal no tenant. Sem tenant_id do client."
    variant="subtle"
  ...
  >
    ...
    >
      Nenhum canário DTE pendente para este Tenant.
    </div>
    ...
    >
      <dl class="grid gap-2 text-sm sm:grid-cols-2">
        ...
        <div>
          ...
          <dd class="font-medium">
            ...
            · Tenant: {{ pending.tenant_admin_approved ? 'sim' : 'não' }}
          </dd>
        </div>
        <div>
          <dt class="text-muted">
      ...
      <UButton
        v-if="isTenantAdmin && !pending.tenant_admin_approved"

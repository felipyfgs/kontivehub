...
async function load() {
  loading.value = true
  try {
    const res = await api.tenantAutXml.cursor()
    cursor.value = res.data.cursors[0] ?? null
...
const cStatHint = computed(() => {
  ...
  if (c === '137') {
    return 'cStat 137: sem documentos novos — espera mínima de 1h (não é falha nem carga retroativa concluída).'
  }
  if (c === '138') {
...
<template>
  ...
  >
    ...
    <div v-else-if="!cursor" class="space-y-3 text-sm text-muted">
      <p>
        Nenhum cursor central ainda. Cadastre o perfil institucional e o certificado na área do escritório.
      </p>
      <UButton
        to="/conta/escritorio"
        color="neutral"

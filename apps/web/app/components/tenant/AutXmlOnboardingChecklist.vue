...
const tenantCnpj = ref<string | null>(null)
const stream = ref<TenantAutXmlStream | null>(null)
const coverage = ref<TenantAutXmlCoverage | null>(null)

const table = useTemplateRef('table')
const feed = usePagedTable<TenantAutXmlEnrollment>({
  getKey: row => row.establishment_id,
  pageSize: 20,
  load: async ({ page, pageSize, signal }) => {
    const epoch = sessionEpoch.value
    const response = await api.tenantAutXml.overview({ page, per_page: pageSize }, { signal })
    if (epoch === sessionEpoch.value) {
      tenantCnpj.value = response.data.tenant_cnpj
...
const columns: TableColumn<TenantAutXmlEnrollment>[] = [
...
function updateEnrollment(updated: TenantAutXmlEnrollment) {
...
async function enroll(row: TenantAutXmlEnrollment) {
  if (!canManageClients.value || acting.value) return
  const epoch = sessionEpoch.value
  const estId = Number(row.establishment_id)
  acting.value = estId
  try {
    const response = await api.tenantAutXml.enroll(estId)
...
async function confirm(row: TenantAutXmlEnrollment) {
  ...
  acting.value = id
  try {
    const response = await api.tenantAutXml.confirm(id)
    if (epoch !== sessionEpoch.value) return
    updateEnrollment(response.data)
...
async function inactivate(row: TenantAutXmlEnrollment) {
  if (!canManageClients.value || acting.value) return
  const epoch = sessionEpoch.value
  const id = Number(row.id)
  if (!id) return
  acting.value = id
  try {
    const response = await api.tenantAutXml.inactivate(id)
    if (epoch !== sessionEpoch.value) return
    updateEnrollment(response.data)
    ...
    <UPageCard variant="subtle">
      <div class="flex flex-wrap items-center gap-3 text-sm">
        <div>
          ...
          <code class="text-highlighted">{{ tenantCnpj || '— cadastre a identidade fiscal —' }}</code>
        </div>
        <UButton
          size="sm"
          color="neutral"

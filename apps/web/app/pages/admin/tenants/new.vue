...
const formError = ref('')
const result = ref<CreatePlatformTenantResult | null>(null)
...
async function submit() {
  ...
  try {
    ...
    const res = await api.platform.tenants.create({
      name: form.name.trim(),
      profile: {
        cnpj: '',
    ...
    if (res.data.credential_delivery === 'regeneration_required') {
      ...
      const id = res.data.tenant?.id
      if (id) {
        await router.replace(`/admin/tenants/${id}`)
        return
      }
    }
...
function goToDetail() {
  const id = result.value?.tenant?.id
  if (id) void router.push(`/admin/tenants/${id}`)
  else void router.push('/admin/tenants')
}
      ...
      >
        <template v-if="result && secretDelivered">
          <UPageCard
            ...
            :description="result.tenant?.name"
            variant="subtle"
            data-testid="admin-tenant-created"
          >
        ...
        <template v-else>
          <div
            class="sm:hidden"
            data-testid="admin-tenant-mobile-progress"
          >
          ...
          <div class="hidden sm:block">
            <UStepper
              ...
              color="primary"
              size="sm"
              data-testid="admin-tenant-stepper"
            />
          </div>
          ...
          <UAlert
            ...
            data-testid="admin-tenant-wizard-error"
          />
          ...
          >
            <div
              v-if="step === 0"
              class="space-y-4"
              data-testid="wizard-step-tenant"
            >
              <UFormField
              ...
              >
                <UInput
                  ...
                  class="w-full"
                  autocomplete="organization"
                  data-testid="wizard-tenant-legal-name"
                />
              </UFormField>
              <UFormField
              ...
              >
                <UInput
                  ...
                  class="w-full"
                  autocomplete="email"
                  data-testid="wizard-tenant-email"
                />
              </UFormField>
              <UFormField
              ...
              >
                <UInput
                  ...
                  class="w-full"
                  autocomplete="tel"
                  data-testid="wizard-tenant-phone"
                />
              </UFormField>
            </div>
